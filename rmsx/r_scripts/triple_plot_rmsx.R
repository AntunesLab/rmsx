...
parse_args <- function() {
  ...
  list(csv_path = args[1],
       ...
       palette = args[6]) # almost there, just need to add it to the command line arg
  }
...
#   palette <- "plasma" # CHANGE
  plot_rmsx(rmsx_long, interpolate, palette)
}

# Function to plot data
plot_rmsx <- function(rmsx_long, interpolate, palette) {
#   palette <- "mako" # CHANGE
  rmsx_plot <- ggplot(rmsx_long, aes(Time_Point, Residue, fill=RMSF)) +
    ...
    # facet_wrap(~Chain) +
    scale_fill_viridis(option = palette) + # was plama by default
...
#     args$triple <- FALSE
    save_plot(rmsx_plot, args$csv_path, id) # what is this doing?
    ...
    if (args$triple == TRUE){
        ...
        png(output_file, width = 800, height = 600)         # Open a PNG device
        grid::grid.draw(rmsx_plot)                        # Draw the plot
